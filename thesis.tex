...
\end{align}
where $\xlast$ is the previous \post state estimate, and $\lastecov$ is the corresponding \post error covariance matrix.
...
Even if the notation in the prediction and the update equations does not explicitly suggest it, multiple predictions can be made recursively using \prior estimates, or multiple updates can be made at each time instance.
...
In the filtering stage, the probability density $\modeobsprob = p\left( \z | M_k = m_i, \mathcal{Z}_{k-1} \right) $ is calculated, which is the probability density of the observation $\z$ when mode $m_i$ is assumed active at time instance $k$ and the observation history up to time instance $k-1$ is given.
...
Therefore, using only the mode probabilities as the states enables fast learning, low Probability to Lose a Track (PLT), and low tracking load.
