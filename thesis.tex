...
The time available for learning the different channels depends on the dynamic behavior of targets and the propagation scenario.
...
The algorithm calculates indexes for each arm that quantify the quality of selecting an arm and emphasizes exploration when needed. Therefore, the exploration--exploitation trade-off is automatically well balanced. 
Those indexes are given for the combinatorial optimization algorithm to replace the channel SINR values to optimize the objective function.
...
The Figure~\ref{fig:non_stationary_ci} demonstrates that the RBE algorithm has a smallest regret with low variance.
...
The equation \eqref{eq:exp_reward_nonmyopic} designates that the expected discounted sum of rewards can be composed of two parts, as also shown in equation \eqref{eq:bellman_action}.
...
The state is obtained by quantizing the continuous values into quantization intervals of equal size with boundaries given by of equal sizes with interval edges given by
...
Important source of information from the tracking filters is the innovation sequence. 
...
Furthermore, the center beam SNR $\sno$ and half of the -3dB beamwidth $B$ are constant in the simulations. 
...
The errors explain why the RL agent had slightly higher risk of losing the track in trajectory 4.
