...
This thesis aims to formulate RRM subproblems as RL problems to identify if learning-based approaches can improve the performance of a radar system when the system can learn from experience. 
...
TX-RX selection is addressed by utilizing the Multi-Armed Bandit (MAB) framework, which is developed for problems solely concentrating on the exploration-exploitation trade-off. 
RIS is formulated as a general RL problem that is solved by using the Q-learning algorithm.
It will be shown that the considered RRM subproblems can be addressed using RL methods and that the performance can be improved by enabling the radar system to learn in real-time.
...
The RL approach for distributed MIMO radar TX-RX selection is formulated and evaluated in Chapter \ref{sec:RL_TX_RX}. Moreover,  Chapter \ref{sec:rl_ri} introduces the RL formulation for the RIS problem, and the approach is evaluated in Monte Carlo simulations. 
...
Typically the RRM problems to be solved depends on the specific radar type. 
Finally, Section \ref{sec:Tracking} covers basic concepts of target tracking because the target tracking is one of the essential radar tasks, which involves many RRM problems.
...
\begin{equation}
    f_d = \frac{2 f_c v_r}{c},
\end{equation}
where $f_c$ is the frequency of the transmitter waveform, $v_r$ is radial velocity towards the receiver, and $c$ is speed of light.
The shift can be utilized for determining moving target echoes, denoted as moving target indication (MTI) from the stationary clutter echoes.
The Doppler shift can also be utilized to measure the range-rate, i.e., the radial velocity of the target.
...
If the target is at a longer range than the maximum unambiguous range, the received pulse is not the most recent transmitted pulse such that the propagation delay $\dt$ could not be correctly calculated.
...
Different kinds of target motion models have been developed to mimic the target motion with a desired accuracy. 
...
Non-maneuvering motion means that the target velocity and elevation remain approximately constant, and otherwise, the motion is considered a maneuver.
...
The process noise covariance matrix can be calculated using equation \eqref{eq:g_noise_mode1} as follows  
\begin{equation}
...
In this section, a measurement model is introduced for the monostatic radars.
...
\end{array}\right.
\end{equation}
In Cartesian coordinate system, the measurement model $h(\x)$ in equation \eqref{eq:spm_obs} can be expressed with linear combinations $h(\x) = \omodel \x$.
...
When the measurement error covariance is high compared to the predicted error covariance, the Kalman gain corrects less the \post estimate compared to the situation where the measurement error covariance is lower when the measurement can be trusted more. 
The Kalman gain is found by minimizing mean square error (MSE) between $\x$ and $\xpost$.
...
An Interacting Multiple Model (IMM) estimator is a specific suboptimal hybrid filter that achieves a significant trade-off between the performance and the computational complexity \cite{Mazor1998}.
...
\begin{equation}\label{eq:epsilon_greedy}
    ...
    \left\{
        \begin{array}{ll}
            \arg\max_{a' \in \As} q_k(s, a') & \text{with probability $1-\epsilon$}\\
...
The majority of the other exploration and exploitation policies that have been proposed in the literature are not suitable for solving general RL problems \cite{Slivkins2019}.
...
The stochastic MAB problem is an MDP with one state \cite{Sutton2018}.
It is different from the Markovian MAB problem in which each machine is modeled with a Markov chain \cite{Katehakis1987}.
...
This thesis proposes the following criterion
\begin{equation}\label{eq:reg}
    L_n = \sum_{t=1}^T \frac{\mu^*_t - \mu^\pi_t}{\mu^*_t},
\end{equation}
...
\end{equation}
where $\leftarrow$ is assignment operator, $r$ is the received reward from selecting the arm $a$, $\alpha \in (0, 1]$ is a step size, and $q(a)$ is an estimate of the action-value $q_{\pi}(a)$ \cite{Sutton2018}.
...
The index captures the uncertainty on the action-value $q_k(a)$ and emphasizes exploration for those arms that might have desirable expected rewards.
The index is constructed in a way that the arm with the highest index is selected.

The UCB1 and KL-UCB algorithms derive upper confidence bound for the expected rewards, and the arm with the highest bound is selected \cite{Sutton2018, Garivier2011}.
...
In addition, the index is constructed so that the agent prefers arms with the highest action-values.
...
In infinite horizon MDPs, the agent can interact with the system for a sufficient number of future states.
...
However, the training required to train a deep RL agent can get quite intensive \cite{Irpan2018}.
...
The following techniques for addressing RRM problems were identified in the literature \cite{Moo2016, Koch1999, Krishnamurthy1999, Krishnamurthy2001, Wintenby2006, LaScala2006, Rajkumar1997, Rajkumar1998, Kastella1997, Kreucher2004, Kreucher2005, Xu2010}.
...
Alternatively, the parameters can be fixed, which could lead to allocating too little or too much radar resources for the radar tasks.
...
The Markov Decision Process (MDP) is a particular case of SDP for which the Markov property is satisfied, and the states and the actions are discrete.
The RRM problem can be interpreted as a decision-making problem in which the decision-maker needs to decide sequentially which radar task is executed next, and how to allocate the radar resources for it \cite{Krishnamurthy1999, Krishnamurthy2001, Wintenby2006, LaScala2006}.
...
\end{equation}
which is absolute value of the innovation $e_k$ normalized with the standard deviation of the measurement noise which is $\sigma_m$.
The equation \eqref{eq:innovation} was derived from the assumption that prediction error at time instance $k$ is proportional to the acceleration and the square of the RI at time instance $k-1$.
...
A target trajectory with constant velocity and $90^\circ$ turn in the middle was used to evaluate the adaptive update interval algorithm.
...
Moreover, it was suggested that significant variations in $e_k$ could be smoothed by using a first-order low-pass filter.
...
\begin{equation} \label{eq:criterion2}
    G(t + \ri | t) \leq V_0 B
\end{equation}
where $G$ stands for the standard deviation of the error along the major axis of ellipsoid in the sine space \cite{vanKeuk1993}.
...
The maneuver parameters $\Theta$ and $\Sigma$ are calculated from the used IMM estimator models and their posterior model probabilities.
...
The TX-RX selection problem will be addressed in Chapter \ref{sec:RL_TX_RX} by applying RL techniques.
...
However, the number of the subsets is $\binom{N}{N_S}\binom{M}{M_S}$ which grows exponentially. This could make the problem unsolvable with the MAB approach because there is no time to explore all the arms in a given time horizon.
...
The normalized regret through the simulation period is shown in Fig~\ref{fig:regret}.
...
From Fig.\ref{fig:stationary_regret}, it can be observed that all algorithms other than $\epsilon$-greedy can achieve sublinear regret.
...
The Fig.\ref{fig:non_stationary_ci} demonstrates that the RBE algorithm has a minimal regret with low variance.
...
However, applying RL techniques to find an optimal solution approximately can be challenging for two reasons
\begin{enumerate}
...
Discretizing the observation space would not help either since the number of discrete observations would grow exponentially as the number of targets increased.

The second difficulty is related to a more general problem with sensing systems,
as the variables derived from the radar measurements always include a certain degree of uncertainty.
However, there are at least a few different ways to tackle the POMDP problems using reinforcement learning.
...
In the latter approach, the RL policy depends on the model; thus, modeling errors can decrease the performance of the policy.
...
For example, RIS algorithms reviewed in Section \ref{sec:tbm_ri}, or the RI could be selected by an RL agent described in the next section.
...
The RL formulation is proposed for algorithms with discrete action and state spaces, such as the Q-learning algorithm has \cite{Sutton2018}. 
Therefore, the action and state spaces' cardinality must be tolerable because otherwise, the exploration would take too much time.
...
Maximum RI $\tmax$ and minimum RI $\tmin$ are loosely defined using expert knowledge or by a trial and error approach in the simulations. 
Loosely defining means that $\tmin$ can be shorter RI than the radar may ever use and similarly $\tmax$ can be longer.
An example set of actions for direct selection is $A_k \in \{0.5, 1.0, 1.5, 2.0, 2.5\}$, where $\tmax=2.5$, $\tmin=0.5$ and size of the action space is $5$.
...
The agent's overall objective is to minimize the time budget needed to maintain a track and prevent track losses based on the cost of a track loss.
The objective should be reflected in the immediate or cumulative rewards that are given for the RL agent, as in equation \ref{eq:discounted_sum}. 
The proposed immediate rewards are
\begin{equation}
    r = \left\{
    \begin{array}{ll}
        -\frac{n}{\ri} \tau, & \text{in case of a detection,}\\
        -\closs, & \text{otherwise}.
    \end{array} \right.
\end{equation}
where $\tau$ is the dwell time and $n$ is number of dwells needed to achieve a detection, and $\closs$ is the cost for losing a track.
In the case of detection, the tracking load is given as a penalty.
In addition, the cost $\closs$ controls the risk of the agent from losing a track while minimizing the load.
...
However, in the myopic case, when $\lambda = 0$, RL algorithms will converge faster.
...
The expected value of a non-myopic action-value is more complicated since the MDP can end if the track is lost.
Therefore, 
\begin{align}
...
The state is obtained by discretizing the continuous variables into bins of equal sizes with bin edges given by
\begin{equation}\label{eq:state_limits}
    e_n = \frac{\ghigh - \glow}{N_d - 2} n + \glow
\end{equation}
...
Two states are defined for the continuous variables below $\glow$ and above $\ghigh$ such that $N_d$ discrete states are obtained.
...
The number of discrete states remains manageable as long as the number of the discrete probabilities and the modes remains sufficiently low. 
...
The RL approach is evaluated using target trajectories from a benchmark problem proposed in \cite{Blair1998}. 
...
The measurement model described in Section \ref{sec:measurement_model} is used to map the states to the measurements in the tracking filter.
In addition, the radar system is using an IMM estimator in which the Constant Velocity (CV) and the Constant Acceleration (CA) models presented in Section \ref{sec:target_models} are used.
The configuration of the IMM estimator will be covered in more detail in Section \ref{sec:IMM}.
...
The motion models the CV and the CA models described in Section \ref{sec:target_models}.
...
Second, only one model probability is sufficient to present the state since the IMM estimator has only two motion models.
Lastly, the CVCA-IMM estimator has only three parameters to be tuned.
These parameters are the acceleration noise variance for CV model $\varcv$, the acceleration noise variance for CA model $\varca$, and the mode transition probability $\msp$.
...
\begin{figure}[bt]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/benchmark/IMM/correlation_imm.pdf}
...
In the ideal case, the auto-correlation function should exhibit a significant correlation, which would indicate that the innovation sequence is approximately non-correlated noise.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.8\textwidth}
        ...
        \caption{Probability to lose a track.}
    ...
    \end{subfigure}
    \caption{The probability to lose a track and mean tracking load evaluated with various track sharpness parameters.}
    \label{fig:track_sharpness_comparison}
\end{figure}
...
\begin{equation}\label{eq:baseline_ineq}
    ...
    \end{array}
\end{equation}
where $V_0$ is the track sharpness parameter, and $\sigma_\theta(t+\ri|t)$ is square root of the error variance of the \prior estimate of the target azimuth angle $\theta$.
The highest RI to satisfy the constraint is found by propagating the predictive equations of the CVCA-IMM estimator with the time step $\dt$.
The track sharpness $V_0$ affects the PLT and needs to be tuned for the specific IMM estimator.
...
The RL approach was evaluated in the simulation setup described in Section \ref{sec:ri_setup}.
...
\begin{itemize}
    ...
    \item evaluate the PLT and tracking load during the learning,
...
One training episode corresponded with a single trajectory simulated until the end of the trajectory or where the target is lost.
Each agent was trained for $4000$ episodes corresponding to training an agent on each trajectory approximately for $667$ times on average.

All the agents used the Q-learning algorithm with \egreedy exploration.
The exploration probability was initially $\epsilon=0.2$ and between each episode, the epsilon was decayed by a factor of $0.99867$, which results in $\epsilon=0.001$ on the last training episode.
The learning rate $\alpha$ was controlled adaptively by using the asynchronous update strategy \cite{Even-Dar2003}.
...
Furthermore, various agents were configured such that it is possible to identify effective state presentations from the states discussed in Section \ref{sec:states}, and to determine if using a myopic policy is sufficient.
The MDP states presented in Section \ref{sec:states} are the innovation sequence and the mode probability, which both can be augmented by the previous RI.
...
The configurations of the agents are shown in Table \ref{tab:agent_configurations}. The ID's from the table are used in the figures later.
...
The target performance and the behavior performance are evaluated using the tracking load in equation \eqref{eq:load} and the PLT.
The mean tracking load is defined as
...
The behavior policies were evaluated every 200 episodes using the data from the past M=200 training episodes to calculate the \eqref{eq:criterion_load} and \eqref{eq:criterion_lost}.
...
Notably, the mean tracking load in Figure \ref{fig:offline_load} meets its lower limit quite fast, but the mean PLT in Figure \ref{fig:offline_load} will take more time to converge to tolerable levels.
...
Therefore, the target policies had fast improvement in the tracking load, as shown in Figure \ref{fig:offline_load}. Meanwhile, the PLT takes more training episodes to converge to low values, as shown in Figure \ref{fig:offline_lost}.
Thus, learning not to lose track will take more training episodes.
The offline mean tracking load rises slightly when the agent has learned more because the agent becomes more conscious about losing track with longer RIs.
...
\begin{table}[tb]
    ...
    \end{tabular}
    \caption{Probability to lose a track for agent policies after 4000 episodes of learning. T1 $\ldots$ T6 denote target trajectories from 1 to 6.}
...
With no exception, the non-myopic agents have a higher tracking load than the corresponding myopic agent with the same state presentation. 
...
First, the non-myopic agents are more conscious of losing track because losing track in the future is also included.
...
The second reason is that for non-myopic policies, the q-values converge slower than for myopic policies. Thus, training an agent to its full potential generally requires more training episodes.
...
In terms of fast learning speed as well as low mean PLT and mean tracking load, the agent with $id=4$ is identified to have desirable performance.
...
Lastly, the agent with $id=4$ is trained and evaluated with various values of $\closs$.
Figure \ref{fig:penalty_comparison} shows PLT for each trajectory and the mean tracking load with using various $\closs$.
As expected, using a lower $\closs$ results in a higher PLT and a lower tracking load.
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{The probability of losing a track and mean tracking load evaluated with various penalties given for losing a track. The relative penalty means that the track lost penalty is divided by the tracking load obtained with the shortest revisit interval and when detection is achieved with single dwell.}
...
An RL agent and the baseline algorithm are compared in the benchmark problem by evaluating the tracking load as defined in equation \eqref{eq:load}.
...
Lastly, RI selections are shown to illustrate differences in the RL agent RI selections and the baseline algorithm RI selections.
...
Using figures \ref{fig:baseline_plt} and \ref{fig:penalty_plt}, the parameters of the RL and the baseline algorithms were tuned to achieve an approximately equal PLT.
Thus, the parameters $\closs = 0.2$ and $V_0=0.164$ were selected.
With the corresponding parameters, the RL agent lost target in trajectory 4 with a probability of $0.5\%$. The baseline algorithm did not lose any tracks in the Monte Carlo simulations with $1200$ iterations.
...
However, the probability of losing track is higher at $0.5\%$ than in other trajectories because the state presentation may not generalize the corresponding trajectory well enough.
It could be possible that the track is lost with a lower probability if the target range would be included in the state.
...
The error explains why the RL agent lost the trajectory four more often than the baseline algorithm.
...
Model probabilities of the IMM estimator and the most recent RIs and innovations were discretized such that those could be used as the states for the Q-learning algorithm.
...
The most efficient state presentation was identified by simulating multiple differently configured  RL agents in the benchmark trajectories.
The agents using the model probabilities had excellent performance in terms of the mean tracking load and the mean PLT.
...
Lastly, it was identified that augmenting the state with the most recent RI does not significantly improve the tracking load performance.
Moreover, the larger state space leads to slower learning speed that causes higher PLT.  

The myopic RL agent using the mode probabilities as states was compared against the baseline algorithm in terms of tracking load, the PLT, and the prediction error.
...
Contextual multi-armed bandits could be used to address the non-stationary reward distributions and enable using the RL approach even with shorter intervals. 
...
Alternative observation presentations were identified based on the innovation sequence, motion model probabilities, and the most recent revisit interval.
...
The learning speed could be improved by applying function approximators to generalize states-action pairs that the agent has not seen before.  
...
In future research, RIS could be extended by applying RL to control dwell times.

Both of the RRM subproblems were evaluated in the single target tracking scenario, such that the RL formulations and the simulation scenarios were less complicated. 
