...
The RRM problem can be divided into numerous subproblems, which are typically solved using models for the radar, the environment, and the target \cite{Moo2016, Koch1999, Krishnamurthy2001, Wintenby2006, LaScala2006, Rajkumar1997, Rajkumar1998, Kastella1997, Kreucher2004, Kreucher2005, Xu2010}.
...
This thesis aims to formulate RRM subproblems as RL problems and propose RL-based algorithms to solve the problems and to identify if RL can improve radar systems' performance. 
...
These problems diversely showcase different RL methods useful for RRM in general.
...
TX-RX subsets are selected to obtain desired spatial diversity gain and simultaneously reduce the cost and power consumption.
...
Chapter \ref{sec:background} introduces principles of radars and target tracking with radars. 
Chapter \ref{sec:RL} reviews the theory on RL that is important to understand the proposed RL approaches. 
Chapter \ref{sec:existing_RRM} explains the considered RRM subproblems in detail and describes the existing solutions.
The RL approach for distributed MIMO radar TX-RX selection is formulated and evaluated in Chapter \ref{sec:RL_TX_RX}. 
Chapter \ref{sec:rl_ri} formulates and evaluates the RL approach for the RIS problem.
...
\end{equation}
which is the probability that system changes from state $s \in \Ss$ to state $s' \in \Ss$ after the action $a \in \As_s$ was taken and agent receives immediate reward $r \in \Rs_{ss'}^a$.
