...
% \newacronym{plt}{PLT}{Probability to Lose a Track}
...
The PLT refers to the probability $\Pr{\text{Track lost}|\pi, i}$ that is the probability to lose a target on a certain trajectory $i$ given the RIS policy $\pi$.
On the other hand, mean PLT refers to the probability
\begin{equation}\label{eq:mean_plt_exp}
...
Therefore, the sampling distribution of the tracks $P_T(i)$ in equations \eqref{eq:mean_load_exp} and \eqref{eq:mean_plt_exp} is a discrete uniform distribution.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.8\textwidth}
        ...
        \caption{Mean PLT. Two distinct clusters can be seen which divide the agents to myopic and non-myopic policies.}
        \label{fig:online_lost}
    \end{subfigure}
    \caption{Behavior policy performance estimated from training data. Myopic agents using the states based on mode probabilities achieve low mean Probability to Lose a Track (PLT) and low tracking load.}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.8\textwidth}
        ...
        \caption{Mean PLT. Achieving a low mean PLT requires multiple training episodes, especially for non-myopic agents.}
        \label{fig:offline_lost}
    \end{subfigure}
    \caption{
        Target policy performance estimated from Monte Carlo simulations. 
        The agent with ID=4 achieves fast learning speed, low mean Probability to Lose a Track (PLT), low mean tracking load.  
    }
...
\begin{table}[tb]
    ...
    \end{tabular}
    \caption{Mean Probability to Lose a Track (PLT) for target policies after 4000 episodes of learning. T1 $\ldots$ T6 denote the employed trajectories from 1 to 6.}
...
Especially from Table \ref{tab:plt_comparison}, it can be seen that after learning for 4000 episodes, the mean PLT of target policies is significantly higher for myopic agents than for non-myopic agents.
...
In terms of fast learning speed, low mean PLT, and low mean tracking load, the agent with $\text{ID}=4$ achieves desirable performance.
...
% \begin{figure}
%     \centering
%     \subcaptionbox{The PLT of the RL agent.\label{fig:penalty_plt}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/plt_agent.pdf}}
...
%     \subcaptionbox{The PLT of the baseline algorithm.\label{fig:baseline_plt}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/plt_baseline.pdf}}
...
%     \subcaptionbox{The PLT of the RL agent.\label{fig:penalty_plt}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/plt_agent.pdf}}
%     \subcaptionbox{The PLT of the baseline algorithm.\label{fig:baseline_plt}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/plt_baseline.pdf}}
%     \caption{The RL agent (ID=4) and the baseline algorithm with different parameterizations in terms of Probability to Lose a Track (PLT).}
%     \label{fig:comparison}
% \end{figure}
...
Figures \ref{fig:penalty_plt} shows the PLT for each benchmark trajectory.
It can be seen that low cost of losing a track results in high PLT, and vice versa.
The baseline algorithm was also evaluated with different track sharpness values.
Figure \ref{fig:baseline_plt} shows the PLT for each benchmark trajectory with different the track sharpness parameter $V_0$ values.
...
Figure \ref{fig:comparison_TL_PLT} shows the mean tracking load as a function of mean PLT for the RL agent and the baseline algorithm. 
...
Especially when selecting small values of $V_0$ to desire a low PLT, a high tracking load can be easily induced.
...
Therefore, using only the mode probabilities as the states enabled fast learning, a low Probability to Lose a Track (PLT), and a low tracking load.
Furthermore, the simulation results indicate that the myopic policies achieved a low tracking load with a desirable PLT, and non-myopic agents obtained a lower PLT, but the tracking load was considerably higher.
