...
\begin{align}
    v_\pi(s) 
    &= \E{ \sum_{i=0}^{\infty} \lambda^i R_{k + i + 1} \bigg| S_k=s} \\
    &= \E{R_{k + 1} + \lambda \sum_{i=0}^{\infty} \lambda^i R_{k + i + 2} \bigg| S_k=s} \\
...
\begin{multline} \label{eq:exp_reward_nonmyopic}
    \E{\sum_{i=0}^\infty \lambda^i R_{t+1} | S_t=s, A_t=a} = \E{R_{t+1} | S_t=s, A_t=a} \\ + (1-\ploss(s, \ri)) \lambda \sum_{s'\in \Ss} p(S_{k+1}=s'|S_{k}=s, A_{k}=a)  V(s')  
\end{multline}
