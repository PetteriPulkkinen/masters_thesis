...
% \newacronym{ct}{CT}{Constant Turn}
% \newacronym{cv}{CV}{Constant Velocity}
% \newacronym{hmm}{HMM}{Hidden Markov Model}
% \newacronym{imm}{IMM}{Interacting Multiple Model}
% \newacronym{klucb}{KL-UCB}{Kullback-Leibler Upper Confidence Bound}
% \newacronym{kp}{KP}{Knapsack Problem}
% \newacronym{mab}{MAB}{Multi-Armed Bandit}
% \newacronym{mdp}{MDP}{Markov Decision Process}
% \newacronym{mimo}{MIMO}{Multiple-Input Multiple-Output}
% \newacronym{pomdp}{POMDP}{Partially Observable Markov Decision Process}
% \newacronym{tlp}{TLP}{Track Loss Probability}
% \newacronym{prf}{PRF}{Pulse Repetition Frequency}
% \newacronym{prt}{PRT}{Pulse Repetition Time}
% \newacronym{qram}{Q-RAM}{Quality of service Resource Management Model}
% \newacronym{rcs}{RCS}{Radar Cross Section}
% \newacronym{rbe}{RBE}{Recency Based Exploration}
% \newacronym{ri}{RI}{Revisit Interval}
% \newacronym{ris}{RIS}{Revisit Interval Selection}
% \newacronym{rl}{RL}{Reinforcement Learning}
% \newacronym{rrm}{RRM}{Radar Resource Management}
% \newacronym{rx}{RX}{Receiver}
% \newacronym{sdp}{SDP}{Stochastic Dynamic Programming}
% \newacronym{sinr}{SINR}{Signal-to-Interference-plus-Noise Ratio}
% \newacronym{snr}{SNR}{Signal-to-Noise Ratio}
% \newacronym{stt}{STT}{Single Target Tracking}
% \newacronym{tbm}{TBM}{Time Budget Management}
...
Vahvistusoppimisongelma ratkaistaan käyttämällä Q-oppimisalgoritmia ja epsilon-ahne (epsilon-greedy) menetelmää. 
...
I want to thank Professor Visa Koivunen and Doctor Tuomas Aittomäki for their support in writing the master's thesis and discussing the essential technical details.
The financial support for this work was provided by Maanpuolustuksen Kannatussäätiö (Foundation under the Ministry of Defence) and SAAB Group. 
Also, the discussions with SAAB researchers, which allowed the improvement of radar time budget management related work, are gratefully acknowledged.
...
$\sigma_m^2$ & measurement noise variance \\ 
$\tau_d$ & dwell time \\
...
Radar Resource Management (RRM) considers managing radar resources such as time, energy, and processing budget as well as selecting operational parameters of the radar \cite{Moo2015}.
...
The general RRM problem can be divided into numerous subproblems, which are typically solved using radar, environment, and target models \cite{Moo2015, Koch1999, Krishnamurthy2001, Wintenby2006, LaScala2006, Rajkumar1997, Rajkumar1998, Kastella1997, Kreucher2004, Kreucher2005, Xu2010}.
...
RL would allow radar systems to learn and improve their performance autonomously in real-time from their experience such that the modeling errors could be avoided. 

Current research for applying RL to RRM problems considers
jamming strategies \cite{Qiang2017, Wang2019, Wang2019a, Zhang2019},
anti-jamming strategies \cite{Kang2018, Ak2020}, 
...
These problems are addressed with a diverse set of different RL methods.
...
In addition, selecting a subset of TXs avoids exposing all the TXs to an adversary, which can be desirable in specific applications.
...
The target range is obtained by measuring the propagation time of a radio wave emitted from a TX, reflected off the target, and received by a RX.
For a monostatic radar which has a TX and a RX at the same location, the range is obtained using the following equation \cite{Curry2011}
\begin{equation}
...
The radars steer the transmitted radio waves into the desired direction, either mechanically by rotating the transmit antenna \cite{Curry2011} or using Electronically Steerable Antennas (ESA) such as Active Electronically Scanned Arrays (AESA), which are based on a phased-array technology \cite{Mailloux2005}.
...
An ESA can be steered in any direction with a relatively short delay within the field of view.
...
Typically, the measurements of the state variables are noisy and none of the state variables can be directly measured.
...
In other words, the accelerations along each coordinate are included into the state as follows  
...
Kalman filtering is a Minimum Mean Square Error (MMSE) state estimation algorithm for linear state-space systems \cite{Zarchan2014}.
The discrete-time linear state-space model is expressed using linear combinations as follows \cite{Zarchan2014}
\begin{align}
...
Kalman filtering is optimal when the state-space model matches the real system, the noises are Gaussian and uncorrelated, and the noise covariances are known \cite{Zarchan2014}.
...
The prediction phase can be expressed with two equations as follows \cite{Zarchan2014},
\begin{align}
...
As a result, following update equations are obtained \cite{Zarchan2014}
...
Even if the notation in the prediction and the update equations do not explicitly show it, multiple predictions can be made recursively using \prior{} estimates, and multiple updates can be made at a single time instance.
...
The equation \eqref{eq:mixing_conditional_prob} can be written in the form
\begin{equation}
...
In some cases, it is reasonable to use negative rewards.
...
In other words, the equation \eqref{eq:markov_property} indicates that the probability of the state transition to state $S_{k+1}$ is only dependent on the current state $S_k$.
...
The key element of an MDP is that the environment in which the agent acts is modeled as a Markov chain.
...
The set of all possible rewards is a subset of real numbers $\Rs \subseteq \real$, and the subset $\Rs_{ss'}^a \subseteq \Rs$ is the set of possible rewards given the state transition and the action.
...
\end{equation}
which is the probability that environment changes from state $s$ to state $s'$ after the action $a$ was taken and the agent receives an immediate reward $r \in \Rs_{ss'}^a$.
...
Therefore, the rest of the chapter will concentrate on infinite horizon MDPs in which the agent desires to maximize the sum of discounted future rewards.
...
For $\lambda \ll 1$, the future rewards are considered only a couple of steps forward.
...
\begin{equation}\label{eq:action_value}
    q_\pi(s, a) = \E{G_k | S_k=s, A_k=a, \pi},
\end{equation}
...
\begin{align}
    v_\pi(s) 
    &= \E{ \sum_{i=0}^{\infty} \lambda^i R_{k + i + 1} \bigg| S_k=s, \pi} \\
    &= \E{R_{k + 1} + \lambda \sum_{i=0}^{\infty} \lambda^i R_{k + i + 2} \bigg| S_k=s, \pi} \\
...
The majority of the other exploration and exploitation policies that have been proposed in the literature are not suitable for solving general RL problems \cite{Lattimore2020}.
...
An estimate of the expected reward is updated each time the agent has pulled an arm.
...
Deterministic algorithms divide the exploration and the exploitation phases into blocks of specific lengths \cite{Lattimore2020}. 
...
The regret of the RBE policy is asymptotically bounded below by a logarithmic function \cite{Oksanen2015}.
...
The learning rate $\alpha$ is typically set constant because values of $q(s', a')$ are non-stationary and the policy may evolve in time.
...
The behavior policy is used to improve the target policy, whereas the target policy is better in achieving higher rewards.
Off-policy RL algorithms have different behavior and target policies.
Typically, a behavior policy implements the exploration--exploitation trade-off, which is required to converge to an optimal policy. 
...
The target policy selects the action with the highest Q-value given the current state.
A typical choice for behavior policy is the \egreedy policy. 
The target policy is proven to converge to the optimal policy if the behavior policy explores actions with a probability larger than zero \cite{Sutton2018}.
...
Modern radar technologies, such as electronically steerable antennas and MIMO systems, have enabled radars to simultaneously carry out multiple radar functions, including target tracking and search for undetected targets.
Each radar function executes one or more tasks, and each radar task may involve multiple looks. 
For example, a tracking task indicates tracking a specific target, and maintaining the track may require updating it with certain intervals.
The look of radar is defined to be uninterruptible and has a finite duration in which radar beam is steered into one or more positions \cite{Moo2015}.
A scheduler is responsible for deciding when each look takes place and which looks are dropped in overload situations \cite{Moo2015}. 
...
Radar Resource Management (RRM) considers scheduling of the radar looks, prioritization of the radar tasks as well as allocating radar resources and selecting operational parameters \cite{Moo2015}. 

The significant radar resources are the time, frequency, energy, and antennas.
...
Networked radars have an additional degree of freedom to allocate and configure individual detectors for the radar tasks \cite{Moo2015}. 
...
Efficient RRM algorithms are required to maximize the radar resource utilization for optimal behavior, quantified by utility functions. 
...
This awareness forms an additional source of information to be used for RRM.
...
The following techniques for addressing RRM problems were presented in the literature \cite{Moo2015, Koch1999, Krishnamurthy2001, Wintenby2006, LaScala2006, Rajkumar1997, Rajkumar1998, Kastella1997, Kreucher2004, Kreucher2005, Xu2010}.

\begin{description}

\item[Rule-based approach.]

Rule-based RRM include approaches in which local objectives are optimized instead of optimizing the global objective \cite{Koch1999}.
Using fixed parameters is a special case of a rule-based approach, but this could lead to allocating too little or too much radar resources for the radar tasks \cite{Hoffmann2014}.
...
The Markov Decision Process (MDP) is a particular case of an SDP for which the Markov property is satisfied, and the states and the actions are discrete \cite{Ross1983}.
...
Additionally, in \cite{Esfahani2012}, locally optimized Quality of Service (QoS) parameters are utilized in allocating a radar time budget to optimize global objectives using a heuristic algorithm.
...
In addition, the range-rate measurements were not considered.
...
\end{equation}
where the operator $\floor*{x}$ takes the highest integer less than or equal to $x$.
...
The algorithm was designed to keep the residual between the target position and the position estimate below a predefined threshold $e_\text{th}$.
...
\end{equation}
where $e_0$ is the desired expected residual, which was calculated from the measurement error covariance matrix, desired threshold $e_\text{th}$, and target priority.
A higher target priority implies that a shorter RI is used to reduce the probability of target moving out from the threshold area defined by $e_\text{th}$.
...
The prediction error covariance matrix is a function of the RI because the state transition and process noise covariance matrices are functions of the RI.
...
In addition, an equation was proposed to calculate the RI efficiently for a target with a known or estimated Singer motion model \cite{RongLi2003} parameters.
...
The sine space consists of $u$, $v$ and $d$ coordinates where $d$ is target range, $u=\cos \beta \sin \theta$ and $v=\sin \beta$ for elevation angle $\beta$ and azimuth angle $\theta$ \cite{Mailloux2005}.
...
\end{equation}
where $\mucv$ and $\muca$ are the probabilities for the CV and CA models, respectively.

Another fast algorithm for RIS was proposed in \cite{MasoumiGanjgah2017}.
...
The RI was controlled recursively such that if the highest maneuvering model has the highest posterior probability, then the RI is decreased by multiplying it with a constant less than one.
On the other hand, if the lowest maneuvering model has the highest posterior probability, then the RI is increased by multiplying it with a constant larger than one.
...
The method was published in \cite{Pulkkinen2020}.

The TX and RX selection for distributed MIMO radars is studied to obtain the desired spatial diversity gain while reducing the power consumption and the amount of data to be processed.
...
The Multi-Armed Bandit (MAB) framework provides policies for selecting actions to balance the exploration--exploitation trade-off while maximizing the employed reward function \cite{Lattimore2020}.
...
The proposed RL algorithm is based on a similar principle proposed in \cite{Mukherjee2012, Kuai2019}. 
However, the approach is generalized for index-based MAB algorithms and non-stationary environments.
In Section \ref{sec:tx_rx_problem}, the TX-RX selection problem for distributed MIMO radars is defined in detail.
The problem is formulated as an MAB problem in Section   \ref{sec:tx_rx_mab_formulation}.
Section \ref{sec:sim} gives numerical examples to be used in the simulations.
In Section \ref{sec:tx_rx_results}, the generalized MAB algorithm is simulated in a radar environment, and the performance of several different MAB algorithms is compared.


\subsection{Problem definition}\label{sec:tx_rx_problem}
...
\subsection{Multi-armed bandit formulation}\label{sec:tx_rx_mab_formulation}
...
An arm could be considered as a subset of TX-RX pairs and the reward is calculated using the reward function \eqref{eq:reward_func}.
However, the number of the subsets is $\binom{\nrx}{\srx}\binom{\ntx}{\stx}$, which grows exponentially as number of RX and TX is increased. This could make the problem unsolvable with the MAB approach because there is no time to explore all the arms in a given time horizon.
The time available for learning the different channels depends on the dynamic behavior of targets and the propagation environment.
...
However, the reward function \eqref{eq:reward_func} is known, which enables using the MAB algorithms developed for the classical MAB problem.
A similar approach is used in \cite{Mukherjee2012, Kuai2019} for MIMO antenna selection in mobile communications to maximize throughput.
The approach in \cite{Mukherjee2012} uses the UCB1 algorithm and the rewards are stationary.
In addition, \cite{Kuai2019} uses Thompson sampling and both stationary and non-stationary rewards are considered.
...
The proposed algorithm for solving the combinatorial MAB problem with a known mapping from the arm rewards to the super arm rewards is presented in Algorithm~\ref{alg:gcmab}.
The algorithm calculates an index for each arm that quantify the quality of selecting an arm and emphasizes exploration when needed. Therefore, the exploration--exploitation trade-off is automatically balanced well. 
These indexes are given for the combinatorial optimization algorithm in place of the channel SINR values to for the optimization of the objective function.
In non-stationary scenarios, the past rewards are weighted less than the most recent ones using a constant learning rate. 
In addition, the exploration parameters need to be discounted with the learning rate parameter.
...
\subsubsection{Employed system configuration}
\label{sec:sys_conf}
...
Therefore, the target RCS model depends on the angles to the receiver and the transmitter.
The dependency on the angles for receiver $n$ and transmitter $m$ is denoted by $\ercs$, which is a product between two scattering coefficients taken from a monostatic target RCS model at the illumination angle and the scattering angle.
...
Hence, the power loss of the target fluctuation $c$ is modeled by the exponential distribution with the scale parameter equal to one such that the probability density function is
\begin{equation}
    f(c) = \left\{
    \begin{array}{ll}
        \exp{-c} & \geq 0 \\
    ...
    \right. .
\end{equation}
The random variable $c$ remains constant between two subsequent time instances.
...
the target scattering coefficient $c$ remains constant through a single measurement period, and the statistical variation of noise and interference powers in the received signals is assumed to be negligible.
...
\begin{figure}[!tb]
    ...
    On average, most of the MAB algorithms obtain rewards close to the maximum.}
    \label{fig:sinr}
\end{figure}

\subsection{Simulation results}\label{sec:tx_rx_results}

Two different scenarios are studied in the simulation examples. 
In a stationary scenario, there is a target that remains stationary at position $(0.5, 0.5)$.
In a non-stationary scenario, a target moves from $(0, 0)$ to $(1, 1)$ on a linear trajectory with a constant velocity.
All other parameters for the environment remain the same in both scenarios.
The five algorithms reviewed in Section~\ref{sec:MAB} are used in the simulations.
...
The MAB algorithms are compared in terms of normalized regret given by the equation \eqref{eq:reg}. 
...
\subsubsection{Stationary target scenario}
...
Therefore, it is possible to achieve a logarithmic regret \cite{Lattimore2020}.
...
\subsubsection{Non-stationary target scenario}

The algorithms adapt to the non-stationary conditions by employing the constant learning rate used to discount the Q-values and the exploration parameters.
Thus, the learning rate is set to $\alpha = 0.002$.
Also, each element of $\vsinrb$ is divided by the value $\max_{n,m} \esinrb$ before calculating the index.
This ensures that enough exploration is done at each time instance even if the scale of the rewards changes over time.
...
The Figure~\ref{fig:non_stationary_ci} demonstrates that the RBE algorithm has a smallest regret with a low variance.
...
The changes in reward distributions will force the agent to switch the arm if the Q-value for the arm under exploitation becomes smaller than the other Q-values.
...
The proposed approach was published in \cite{Pulkkinen2020}.


\newpage
\section{Reinforcement Learning Approach for Revisit Interval Selection}\label{sec:rl_ri}

An essential Radar Resource Management (RRM) problem in multifunction radars is determining how to allocate and schedule time among different radar tasks under a limited time budget.
This problem is called the Time Budget Management (TBM) problem, as discussed in Chapter \ref{sec:existing_RRM}.
Adaptive electronically steerable arrays used in modern radars provide flexibility to execute radar tasks in an arbitrary order because steering the radar beam can be done rapidly.
...
In Section \ref{sec:RRM_tech}, different techniques for RRM were presented.
For RL methods, addressing the TBM problem using the Stochastic Dynamic Programming (SDP) technique seems appealing because the RL algorithms are used to solve MDPs.
However, applying RL algorithms to find even an approximate solution for the SDP can be challenging because of two reasons:
\begin{enumerate}
    \item the number of targets may change in time, and
...
The second difficulty is related to a more general problem with sensing systems; the quantities estimated from the radar measurements always include a certain degree of uncertainty.
...
The performance of the RL approach is investigated in Monte Carlo simulations by evaluating the Track Loss Probability (TLP) \cite{Blair1998} and the tracking load.
...
Therefore, the RI $t_r$ is an integer multiple of $\delta t$.
...
\item\textbf{If the target is detected, update the state estimate. If no detection is achieved, either return to step 1 if the number subsequent dwells is below a threshold $\nmax$ or otherwise flag the target lost and exit the tracking loop.}

In the latter case, the target track is considered lost, and it is not required to address here how the radar responds to such a scenario. 
...
\item\textbf{Select the Revisit Interval $\ri$.}
...
The other approach either decreases or increases the RI with some delta ($\Delta$) value, so this approach is called delta selection.
...
In order to obtain a deeper understanding of the rewards, their expected values are examined.
...
\begin{equation}\label{eq:exp_reward}
    \E{R_{t+1} | S_t=s, A_t=a} = -(1-\ploss(s, \ri)) 
        ...
        - \ploss(s, \ri) \closs
\end{equation}
where $\ploss(s, \ri)$ is the probability of losing a target when in state $s$ and using the RI $\ri$, and $\E{n_d|s, \ri}$ is expected value of the number of dwells.
Equation \eqref{eq:exp_reward} gives also the action-value in myopic case.
The non-myopic action-value is more complicated since the MDP can transition to the terminal state if the target is lost.
Therefore, the expected discounted sum of rewards is
\begin{multline} \label{eq:exp_reward_nonmyopic}
    \E{\sum_{i=0}^\infty \lambda^i R_{t+1} | S_t=s, A_t=a, \pi} = \E{R_{t+1} | S_t=s, A_t=a} \\ + (1-\ploss(s, \ri)) \lambda \sum_{s'\in \Ss} p(S_{k+1}=s'|S_{k}=s, A_{k}=a)  V(s')  
\end{multline}
where $V(s')$ is the value, presented in equation \eqref{eq:value}, of state $s'$.
The equation \eqref{eq:exp_reward_nonmyopic} indicates that the expected discounted sum of rewards can be composed of two parts, as also shown in equation \eqref{eq:bellman_action}.
The former part is the expected immediate reward, and the latter part is the expected discounted sum weighted by the discount factor $\lambda$.
In addition, the latter part is also weighted by the probability of not losing a target $(1-\ploss(s, \ri))$.
...
The state is obtained by quantizing the continuous values into quantization intervals of equal size with boundaries given by
\begin{equation}\label{eq:state_limits}
    \frac{\ghigh - \glow}{N_d - 2} n + \glow,
\end{equation}
where $N_d-2$ is number of the intervals, $n \in \{0, 1, ..., N_d-2\}$ is the index of a boundary, and
the values $\glow$ and $\ghigh$ define the lower and the upper limits for the interval boundaries.
...
An important source of information from the tracking filters is the innovation sequence. 
It includes information about how well the tracker predicted the target position compared to the position measurement. 
In addition, range-rate can be examined in radars with a Doppler capability. 
...
The size of the state space can be further reduced by using only the innovation in the angle of the target position, as it can be assumed that the target range is less critical for steering the radar beam in the correct direction.
...
\begin{table}[tb]
    ...
    Beam center SNR (linear)             & $\sno$          & $50$ & -          \\ \hline
...
The developed RL approach for the RIS problem is evaluated in Monte Carlo simulations using simulation setup described in this section.
...
\begin{equation} \label{eq:offset_snr}
    SNR = \sno~\exp{ - \ln{2}
        \frac
            ...
            {B^2}},
\end{equation}
where $\sno=50$ is the beam center SNR in a linear scale, $B=0.02$ is half of the -3dB beamwidth in radians, $\theta$ is target angle and $\hat{\theta} $ is the boresight angle. 
Furthermore, the beam center SNR $\sno$ and half of the -3dB beamwidth $B$ are constant in the simulations. 
...
The monostatic radar is simulated in a tracking scenario using six different trajectories presented in Section \ref{sec:benchmark_trajectories}.
...
The target trajectories used in the simulations are based on a benchmark problem that was initially described in \cite{Blair1998}.
...
For the sake of simplicity, the spatial dimension is reduced from three-dimensional space to 2-D space.
...
The trajectories include different kinds of motion, such as constant-g turns, and straight forward motion with constant or non-constant velocity.
...
The benchmark trajectories are further summarized in Table \ref{tab:benchmark_table}, which contains the mean and the standard deviation values for the acceleration and speed in each trajectory.
...
This configuration is simple yet effective for illustrating the performance of the method.
...
Such a configuration can be justified by three arguments.
...
Secondly, only one mode probability is sufficient to present the state of an MDP based on the mode probability since the IMM estimator has only two motion models.
Lastly, the CVCA-IMM estimator has only three parameters to be tuned.
These parameters are the acceleration noise variance for the CV model denoted by $\varcv$, the acceleration noise variance for the CA model $\varca$, and the mode transition probability $\msp$.

However, there is an issue that needs to be considered when using the two target models.
...
In this thesis a simpler approach is employed, which was also discussed in \cite{Granstroem2015}.
This approach augments the state estimates and corresponding error covariance matrices of the CV model with zeros.
According to \cite{Granstroem2015}, it corresponds to the situation in which the distributions of the accelerations on the x- and y-axes are approximated with the Dirac delta function.
...
As described in \cite{Simeonova2002}, higher transition probabilities lead to lower peak square errors in the predicted states but additionally increases the mean square error.
...
In the ideal case, the innovation sequence should be uncorrelated.
...
\subsubsection{Baseline algorithm used in comparisons} \label{sec:baseline_algorithm}

A baseline algorithm is utilized to compare the RL approach against a conventional RIS algorithm.
The baseline algorithm for controlling the RI is based on an approach proposed in \cite{Daeipour1994}.
This algorithm is prediction error covariance matrix based algorithm that optimizes
\begin{equation}\label{eq:baseline_ineq}
    ...
    \end{array}
\end{equation}
where $V_0$ is the track sharpness parameter, and $\sigma_\theta(t+\ri|t)$ is the standard deviation of the error in the predicted azimuth angle.
...
The track sharpness $V_0$ affects the risk of losing a track and needs to be tuned for the specific IMM estimator.
...
The direct selection action space $\Asdir$ is used in which the number of actions $N_a=10,$ the minimum RI $\tmin=0.1s$, and the maximum RI $\tmax=7.5s$.
State variables used for the agents are the most recent innovation, the probability of the CV mode, and the previous RI.
The discretization parameter was set to $N_d=10$, such that the number of states $N_s=12$ for the state spaces that does not use the previous RI, and $N_s=102$ otherwise.
...
Mean tracking load refers to the following quantity
\begin{equation}\label{eq:mean_load_exp}
...
The ability to track targets without losing them is measured in terms of Track Loss Probability (TLP).
The TLP refers to the probability $\Pr{\text{Track lost}|\pi, i}$ that is the probability of losing a target on a certain trajectory $i$ given the RIS policy $\pi$.
On the other hand, mean TLP refers to the probability
\begin{equation}\label{eq:mean_tlp_exp}
    P_\text{mean} = \Pr{\text{Track lost}|\pi, P_T(i)}
\end{equation} 
that is similar to the TLP but the trajectories are sampled from the distribution $P_T(i)$.
Therefore, the mean TLP is evaluated in Monte Carlo simulations as follows
\begin{equation}\label{eq:criterion_lost}
\   P_\text{mean} \approx \frac{1}{\Ne} \sum_{m=0}^{\Ne-1} \vec{1}_{\text{TrackLost}(m)},
\end{equation}
where indicator function $\vec{1}_{\text{TrackLost}(m)}$ is one if the track is lost in episode $m$ and zero otherwise.
...
\begin{itemize}
    ...
    \item evaluate the mean TLP and the mean tracking load during the learning,
    \item evaluate the amount of learning required to achieve a certain level of performance,
    \item determine the connection between $\closs$ and the TLP, and
...
Therefore, the sampling distribution of the tracks $P_T(i)$ in equations \eqref{eq:mean_load_exp} and \eqref{eq:mean_tlp_exp} is a discrete uniform distribution.
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.72\textwidth}
        ...
        \caption{Mean tracking load. Myopic agents have lower mean tracking load compared to the non-myopic agents.}
        \label{fig:online_load}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.72\textwidth}
        \includegraphics[width=\textwidth]{figures/benchmark/Training/online_plt.pdf}
        \caption{Mean TLP. Two distinct clusters can be seen which divide the agents to myopic and non-myopic policies.}
        \label{fig:online_lost}
    \end{subfigure}
    \caption{Behavior policy performance estimated from training data. Myopic agents using the states based on mode probabilities achieve low mean Track Loss Probability (TLP) and low tracking load.}
...
\begin{figure}
    ...
    \begin{subfigure}[b]{0.8\textwidth}
        ...
        \caption{Mean tracking load. A low mean tracking load is achieved with a small number of training episodes.}
        \label{fig:offline_load}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.8\textwidth}
        \includegraphics[width=\textwidth]{figures/benchmark/Training/offline_plt.pdf}
        \caption{Mean TLP. Achieving a low mean TLP requires multiple training episodes, especially for non-myopic agents.}
        \label{fig:offline_lost}
    \end{subfigure}
    \caption{
        Target policy performance estimated from Monte Carlo simulations. 
        The agent with ID=4 achieves fast learning speed, low mean Track Loss Probability (TLP), and low mean tracking load.  
    }
...
Especially, the mean tracking loads in Figure \ref{fig:offline_load} reach stable levels quite fast, but the mean TLP in Figure \ref{fig:offline_lost} requires more time to converge to tolerable levels.
...
Meanwhile, the TLP takes more training episodes to converge to low values, as shown in Figure \ref{fig:offline_lost}.
...
\begin{table}[tb]
    ...
    \end{tabular}
    \caption{Mean Track Loss Probability (TLP) for target policies after 4000 episodes of learning. T1 $\ldots$ T6 denote the employed trajectories from 1 to 6.}
...
Especially from Table \ref{tab:tlp_comparison}, it can be seen that after learning for 4000 episodes, the mean TLP of target policies is significantly higher for myopic agents than for non-myopic agents.
...
In terms of fast learning speed, low mean TLP, and low mean tracking load, the agent with $\text{ID}=4$ achieves desirable performance.
The agent uses a myopic policy and the mode probabilities as states.

\subsubsection{Performance against the baseline algorithm} \label{sec:against_baseline}

% \begin{figure}
%     \centering
%     \subcaptionbox{The TLP of the RL agent.\label{fig:penalty_tlp}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/plt_agent.pdf}}
%     \subcaptionbox{The mean tracking load of the RL agent.\label{fig:penalty_load}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/tracking_load_agent.pdf}}
%     \subcaptionbox{The TLP of the baseline algorithm.\label{fig:baseline_tlp}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/plt_baseline.pdf}}
%     \subcaptionbox{The mean tracking load of the baseline algorithm.\label{fig:baseline_load}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/tracking_load_baseline.pdf}}
%     \caption{The RL agent (ID=4) and the baseline algorithm compared with different parameterizations in terms of Track Loss Probability (TLP) and mean tracking load.}
...
%     \subcaptionbox{The TLP of the RL agent.\label{fig:penalty_tlp}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/plt_agent.pdf}}
%     \subcaptionbox{The TLP of the baseline algorithm.\label{fig:baseline_tlp}}{\includegraphics[width=0.45\textwidth]{figures/benchmark/Simulations/plt_baseline.pdf}}
%     \caption{The RL agent (ID=4) and the baseline algorithm with different parameterizations in terms of Track Loss Probability (TLP).}
%     \label{fig:comparison}
% \end{figure}
...
\begin{figure}[!tb]
    \centering
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{figures/benchmark/Simulations/plt_agent.pdf}
        \caption{The TLP of the RL agent.}
        \label{fig:penalty_tlp}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \includegraphics[width=\textwidth]{figures/benchmark/Simulations/plt_baseline.pdf}
        \caption{The TLP of the baseline algorithm.}
        \label{fig:baseline_tlp}
    \end{subfigure}
    \caption{
        Target policy performance estimated from Monte Carlo simulations. 
        The agent with ID=4 achieves fast learning speed, low mean Track Loss Probability (TLP), low mean tracking load.  
    }
    \label{fig:comparison}
\end{figure}

The most promising RL agent (ID=4) was trained and evaluated with various values of $\closs$.
Figures \ref{fig:penalty_tlp} shows the TLP for each benchmark trajectory.
It can be seen that low cost of losing a track results in high TLP, and vice versa.
The baseline algorithm was also evaluated with different track sharpness values.
Figure \ref{fig:baseline_tlp} shows the TLP for each benchmark trajectory with different the track sharpness parameter $V_0$ values.
The figures show that maintaining the track of trajectory 6 is the most difficult for the baseline algorithm, while the trajectory 4 is the most challenging track for the RL algorithm.
...
\begin{figure}[!htb]
    ...
    \caption{The mean tracking load as a function of mean Track Loss Probability (TLP) for the baseline algorithm and the RL agent (ID=4). The labels for the data points are the track sharpness parameter values and the cost of losing a track for the baseline algorithm and RL agent, respectively. The RL agent achieves a lower tracking load than the baseline algorithm with mean TLP values below 0.02.}
    \label{fig:comparison_TL_TLP}
\end{figure}

Figure \ref{fig:comparison_TL_TLP} shows the mean tracking load as a function of mean TLP for the RL agent and the baseline algorithm. 
The figure uses the same data that was used to generate Figure \ref{fig:comparison}.
It can be seen that the RL approach achieves a lower tracking load than the baseline algorithm when evaluated with mean TLP values below 0.02. 
However, for mean TLP values approximately above 0.02, the RL agent's mean tracking load is slightly higher compared to the baseline algorithm. 
In other words, the RL agent is more reliable in maintaining the tracks with a low tracking load. 
It can also be seen that the baseline algorithm is quite sensitive to the selection of $V_0$ parameter. 
Especially when selecting small values of $V_0$ to desire a low TLP, a high tracking load can be easily induced.


Using figures \ref{fig:penalty_tlp} and \ref{fig:baseline_tlp}, the parameters of the RL agent and the baseline algorithm were tuned to achieve an approximately equal TLP to evaluate the tracking load in equation \eqref{eq:load}, the selected RIs, and Euclidean distances between the target positions and the predicted target positions.
Thus, the parameters $\closs = 0.2$ and $V_0=0.164$ were selected.
With the corresponding parameters, the RL agent lost the target track in trajectory 4 once in 200 Monte Carlo iterations. 
...
For trajectories 1, 5, and 6, the tracking loads of the baseline algorithm and the RL agent are approximately equal when the target is not maneuvering.
...
The relation between the RI and the target range in conventional RIS algorithms can be seen, for example, from equation \eqref{eq:van_keuk_revisited}.
...
The RL agent is more eager to take the risk of using longer RIs because the experience indicates that using such RIs does not have a high risk of losing track.
However, the TLP is at $0.5\%$, which is higher compared to the other tracks because the state space may not generalize the corresponding trajectory well enough.
...
The significant difference between the baseline algorithm and the RL agent is that the baseline algorithm often used shorter RIs when the target maneuvered sharply and therefore the tracking load was increased while both of the methods still maintained the tracks.
...
The errors explain why the RL agent had a slightly higher risk of losing the track in trajectory 4.
...
Thus the risk of losing the track is not increased.
...
The agents using the mode probabilities had excellent performance in terms of the mean tracking load and the mean TLP.
...
However, the non-myopic agents' ability to consider the long-term consequences resulted in lower TLP. 
...
Moreover, the larger state space leads to slower learning speed that causes higher TLP.  

The myopic RL agent using the mode probabilities as states was compared against the baseline algorithm in terms of the tracking load, the TLP, and the position prediction error.
It was found that the mean tracking load of the RL agent is lower compared to the baseline algorithm when evaluated with low mean TLP values.
Thus the tracks were maintained with with a lower tracking load and longer revisit intervals.
Especially, the proposed learning-based approach benefits from utilizing the experience to reduce the tracking load peaks.
...
RL enables the radar systems to learn in real-time from experience without using a problem model developed beforehand.
...
In this thesis, a method based on the Multi-Armed Bandit (MAB) framework was developed to formulate the TX-RX selection problem as an RL problem.
...
The proposed MAB algorithm actively learns to select a subset of TXs and RXs that maximize the reward function.
...
The RL agent's selections were driven by a reward function that calculates the mean SINR in a linear scale.
...
The developed RL methods based on the KL-UCB and RBE algorithms achieved nearly optimal performance in both stationary and non-stationary scenarios and outperformed the exploration-free method.
...
Adaptive Revisit Interval Selection (RIS) algorithms are used to decrease the time budget allocated for target tracking tasks in a multifunction radar.
In this thesis, the RIS problem was formulated as a Markov decision process (MDP) with unknown state transition probabilities and rewards, and the Q-learning algorithm with \egreedy exploration was used to solve the proposed RL problem.
Two different action spaces, named as direct selection and delta selection, were identified for selecting the Revisit Interval (RI).
In direct selection, the RI is selected directly from a set of different RIs. 
In delta selection, the action is chosen from a set of delta values that either increase or decrease the RI.
Alternative state spaces were developed based on the innovation sequence, the mode probabilities of an IMM estimator, and the most recent revisit interval. 
The innovation sequence contains information about how well the agent predicted the target position and thus can be used to control the revisit interval. 
The mode probabilities are used as states because they contain information about target maneuverability.
The most recent revisit intervals were augmented to the state presentations to attempt to approximate the MDP states more accurately.
The rewards were proposed to quantify the cost of losing a track while emphasizing the agent to minimize the tracking load. 
The proposed immediate rewards are comprised of two components such that both tracking load and losing tracks are penalized.
...
The simulation results demonstrate that the learning speed was significantly decreased when the number of states was increased.
Therefore, using only the mode probabilities as the states enabled fast learning, a low Track Loss Probability (TLP), and a low tracking load.
Furthermore, the simulation results indicate that the myopic policies achieved a low tracking load with a desirable TLP, and non-myopic agents obtained a lower TLP, but the tracking load was considerably higher.

The myopic RL agent using the mode probabilities as states was compared against the baseline algorithm in terms of the tracking load, the TLP, and the position prediction error.
It was observed that the proposed RL agent was better in maintaining the tracks with a low tracking load and longer revisit intervals. 
Furthermore, the position prediction error was not significantly larger compared to the baseline algorithm.
The RL agent achieved a reduction of the tracking load peaks, which is essential when operating in overload situations.

The RL approach for the RIS could be improved in several different ways.
...
In future research, the RIS could also be extended by applying RL to control the dwell times.
...
In future research, extensions to the multi-target tracking scenario should be considered. 
